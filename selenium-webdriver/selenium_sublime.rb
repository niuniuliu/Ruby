...
def start_to_refresh(string_email)
  dr = Selenium::WebDriver.for :chrome
  url = "http://webapplog.com/giveaways/sublime/?lucky=574"
  dr.navigate.to url

  dr.find_element(:id, 'giveaways_answer').click

  ddls = dr.find_elements(:tag_name, "option" )

  ddls.each {|option| option.click if option.text == "Sublime Text"}

  dr.find_element(:id, 'giveaways_email').click()

  puts "*** Start to input email: " + string_email
  dr.find_element(:id, 'giveaways_email').send_keys(string_email)

  dr.find_element(:xpath, '//*[@id="giveaways_form"]/div[3]/div/button').click()
  puts "*** DONE: " + string_email
  puts "*** The URL is #{dr.current_url}"
  dr.close
...
while i < 50000
  random = rand(36 ** 8).to_s(36) + "@gmail.com"
  p random
  start_to_refresh(random)
  i += 1
end
# for option in ddl
#   p option.text
...
require 'selenium-webdriver'
  ...
  # by tagname
  # puts dr.find_element(:tag_name, 'form')[:class_name]
  sleep 2
